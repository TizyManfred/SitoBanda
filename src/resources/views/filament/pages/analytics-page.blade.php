<x-filament-panels::page>
    @php
        $chartWidth = 1000;
        $chartHeight = 260;
        $chartMax = max(array_merge($chartVisitors, $chartPageViews, [1]));
        $chartCount = max(count($chartLabels), 1);
        $visitorsPoints = [];
        $pageViewsPoints = [];

        foreach ($chartLabels as $index => $label) {
            $x = $chartCount > 1 ? ($index / ($chartCount - 1)) * $chartWidth : $chartWidth / 2;
            $visitorsPoints[] = round($x, 2).','.round($chartHeight - (($chartVisitors[$index] ?? 0) / $chartMax * $chartHeight), 2);
            $pageViewsPoints[] = round($x, 2).','.round($chartHeight - (($chartPageViews[$index] ?? 0) / $chartMax * $chartHeight), 2);
        }
        ...
        $summaryCards = [
            ['label' => __('filament.analytics.cards.visitors_7d'), 'value' => $summary['visitors7d'], 'icon' => 'users', 'tone' => 'violet'],
            ['label' => __('filament.analytics.cards.pageviews_7d'), 'value' => $summary['pageViews7d'], 'icon' => 'eye', 'tone' => 'sky'],
            ['label' => __('filament.analytics.cards.visitors_30d'), 'value' => $summary['visitors30d'], 'icon' => 'users', 'tone' => 'amber'],
            ['label' => __('filament.analytics.cards.pageviews_30d'), 'value' => $summary['pageViews30d'], 'icon' => 'eye', 'tone' => 'emerald'],
        ];
    @endphp
    ...
    <style>
        ...
        .analytics-row-meta { overflow: hidden; margin-top: .1rem; color: rgb(156 163 175); font-size: .75rem; text-overflow: ellipsis; white-space: nowrap; }
    ...
    <div class="analytics-grid">
        <section class="analytics-kpis">
            @foreach ($summaryCards as $card)
                <article class="analytics-card analytics-kpi" style="--accent: {{ match ($card['tone']) { 'sky' => '#0ea5e9', 'amber' => '#f59e0b', 'emerald' => '#10b981', default => '#8b5cf6' } }}">
                    ...
                    <p class="analytics-kpi-value">{{ number_format($card['value']) }}</p>
                </article>
            @endforeach
        </section>
                ...
                <div class="analytics-chart">
                    <svg viewBox="0 0 {{ $chartWidth }} {{ $chartHeight }}" preserveAspectRatio="none" role="img" aria-label="{{ __('filament.analytics.traffic.description') }}">
                        <g class="analytics-chart-grid">
                            <line x1="0" y1="0" x2="1000" y2="0"/><line x1="0" y1="65" x2="1000" y2="65"/>
                            <line x1="0" y1="130" x2="1000" y2="130"/><line x1="0" y1="195" x2="1000" y2="195"/><line x1="0" y1="260" x2="1000" y2="260"/>
                        </g>
        ...
        <section class="analytics-two-col">
            <article class="analytics-card analytics-section">
                ...
                <div class="analytics-list">
                    @forelse ($topPages as $page)
                        <a href="{{ $page['url'] }}" target="_blank" rel="noreferrer" class="analytics-row" style="--width: {{ round(($page['page_views'] / $maxPageViews) * 100) }}%">
                            ...
                            <span class="analytics-row-value">{{ number_format($page['page_views']) }}</span>
                        </a>
                    @empty
                        <div class="analytics-empty">{{ __('filament.analytics.messages.no_data') }}</div>
                    @endforelse
            ...
            @foreach ([
                ['title' => __('filament.analytics.tables.top_referrers'), 'items' => $topReferrers, 'label' => 'referrer', 'max' => $maxReferrerViews],
                ['title' => __('filament.analytics.tables.top_browsers'), 'items' => $topBrowsers, 'label' => 'browser', 'max' => $maxBrowserViews],
                ['title' => __('filament.analytics.tables.top_countries'), 'items' => $topCountries, 'label' => 'country', 'max' => $maxCountryViews],
            ] as $ranking)
                <article class="analytics-card analytics-section">
                    <h2 class="analytics-heading">{{ $ranking['title'] }}</h2>
                    <div class="analytics-list">
                        @forelse ($ranking['items'] as $item)
                            <div class="analytics-row" style="--width: {{ round(($item['page_views'] / $ranking['max']) * 100) }}%">
                                <span class="analytics-row-bar"></span>
                                <span class="analytics-row-main"><span class="analytics-row-title">{{ $item[$ranking['label']] }}</span></span>
                                <span class="analytics-row-value">{{ number_format($item['page_views']) }}</span>
                            </div>
                        @empty
                            <div class="analytics-empty">{{ __('filament.analytics.messages.no_data') }}</div>
                        @endforelse
